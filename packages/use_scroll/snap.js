...
function getNextElementSnapPoint(scrollObj, obj, direction) {
  ...
  if (
    (left > 0 && left + getWidth(scrollObj) === getScrollWidth(scrollObj)) ||
    (top > 0 && top + getHeight(scrollObj) === getScrollHeight(scrollObj))
  ) {
    currentIteration = l - 1;
    const lastSnapElement = obj.snapElements[currentIteration];
    const lastSnapCoords = {
      x:
        getLeft(lastSnapElement) -
        getLeft(scrollObj) +
        getXSnapLength(lastSnapElement, lastSnapElement.scrollSnapAlignment.x, direction),
      y:
        getTop(lastSnapElement) -
        getTop(scrollObj) +
        getYSnapLength(lastSnapElement, lastSnapElement.scrollSnapAlignment.y, direction),
    };
  ...
  const currentSnapCoords = {
    x:
      currentIteration === 0
        ? 0
        : getLeft(currentSnapElement) -
          getLeft(scrollObj) +
          getXSnapLength(currentSnapElement, currentSnapElement.scrollSnapAlignment.x, direction) -
          getXSnapLength(scrollObj, currentSnapElement.scrollSnapAlignment.x, direction),
    y:
      currentIteration === 0
        ? 0
        : getTop(currentSnapElement) -
          getTop(scrollObj) +
          getYSnapLength(currentSnapElement, currentSnapElement.scrollSnapAlignment.y, direction) -
          getYSnapLength(scrollObj, currentSnapElement.scrollSnapAlignment.y, direction),
  };
  ...
  let i;
  for (i = currentIteration + primaryDirection; i < l && i >= 0; i += primaryDirection) {
    ...
    snapCoords = {
      y:
        i === 0
          ? 0
          : getTop(currentIteratedObj) -
            getTop(scrollObj) +
            getYSnapLength(
              currentIteratedObj,
              currentIteratedObj.scrollSnapAlignment.y,
              direction
            ) -
            getYSnapLength(scrollObj, currentIteratedObj.scrollSnapAlignment.y, direction),
      x:
        i === 0
          ? 0
          : getLeft(currentIteratedObj) -
            getLeft(scrollObj) +
            getXSnapLength(
              currentIteratedObj,
              currentIteratedObj.scrollSnapAlignment.x,
              direction
            ) -
            getXSnapLength(scrollObj, currentIteratedObj.scrollSnapAlignment.x, direction),
    };
    ...
    if (
      (direction.x === 1 ? left < xThreshold : left > xThreshold) &&
      (direction.y === 1 ? top < yThreshold : top > yThreshold)
    ) {
      break;
    }

    const elementXThreshold =
      snapCoords.x + direction.x * getWidth(currentIteratedObj) * CONSTRAINT;
    const elementYThreshold =
      snapCoords.y + direction.y * getHeight(currentIteratedObj) * CONSTRAINT;

    // check if not scrolled past element snap point
    if (
      (direction.x === 1 ? left > elementXThreshold : left < elementXThreshold) ||
  ...
  return {
    y: stayInBounds(
      ...
      adjustForPadding(obj.snapElements[currentIteration].snapCoords.y, pTop)
    ),
    x: stayInBounds(
      0,
      getScrollWidth(scrollObj),
      adjustForPadding(obj.snapElements[currentIteration].snapCoords.x, pLeft)
    ),
  };
}
