const getVisibleAmount = (element, ancestor) => {
  const distanceX =
    Math.min(ancestor.left, element.left) -
    Math.max(ancestor.left + ancestor.width, element.left + element.width);
  const distanceY =
    Math.min(ancestor.top, element.top) -
    Math.max(ancestor.top + ancestor.height, element.top + element.height);
